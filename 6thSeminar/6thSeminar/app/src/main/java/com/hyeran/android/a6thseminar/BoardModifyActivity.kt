...
import kotlinx.android.synthetic.main.activity_board_modify.*
...
class BoardModifyActivity : AppCompatActivity() {
    ...
    private fun setOnBtnClickListener() {
        btn_album_modify.setOnClickListener {
            // 앨범 여는 로직
        ...
        btn_complete_modify.setOnClickListener {
            putModifyBoardResponse()
            finish()
        }
    }

    // 데이터들을 RequestBody 객체로 만들어서 서버로 요청을 보내는 코드
    private fun putModifyBoardResponse() {
        val input_title = et_title_modify.text.toString()
        val input_contents = et_content_modify.text.toString()
//        if (input_title.isNotEmpty() && input_contents.isNotEmpty()) {
            ...
            // 본격적인 통신
            val putModifyBoardResponse = networkService.putModifyBoardResponse(token, contentIdx, title, contents, mImage)

            putModifyBoardResponse.enqueue(object : Callback<PutModifyBoardResponse> {
                override fun onFailure(call: Call<PutModifyBoardResponse>, t: Throwable) {
                    Log.e("<게시판-게시글 수정> 통신 fail", t.toString())
                }

                override fun onResponse(call: Call<PutModifyBoardResponse>, response: Response<PutModifyBoardResponse>) {
                    if (response.isSuccessful) {
                        toast(response.body()!!.message)
                        finish()
                    } else {
                        Log.e("<게시판-게시글 수정> 응답 fail: ", response.code().toString())
                        Log.e("<게시판-게시글 수정> 응답 fail: ", response.errorBody().toString())
                    }
                }
            })
//        }
        ...
        if (requestCode == REQUEST_CODE_SELECT_IMAGE) {
            if (resultCode == Activity.RESULT_OK) { // 이 두 부분은 형식적
                data?.let {
                    ...
                    Glide.with(this@BoardModifyActivity).load(selectedPictureUri).thumbnail(0.1f).into(iv_photo_modify)
                }
            }
        }
    }
